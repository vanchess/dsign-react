...
class DnNewMessage extends React.Component {
    ...
    _handleSubmit = () => {
        ...
        messageService.sendMsg(msg).then(
            (data) => { 
                ...
                this.props.history.push(`/dn/list/${msg.type}/${data.id}`) 
            },
            (err) => { 
                alert(err);
                this.setState({
...
export default connect(mapStateToProps,mapDispatchToProps)(withRouter(DnNewMessage));