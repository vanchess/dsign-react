...
import { messageFetch } from '../../bill/rejectedOutAction.js'

export const messageChangeRowPerPage = (newPerPage) => {
    return {type: REJECTED_OUT_CHANGE_PER_PAGE, perPage: newPerPage}
}

export const messageChangePage = (newPage) => {
    return {type: REJECTED_OUT_CHANGE_PAGE, page: newPage}
}

export const messageStartChangeRowPerPage = (newPerPage) => {
    return (dispatch, getState) => {
        const state = getState();
        let currentPerPage = state.paginationReducer.bill.rejectedOut.perPage;
...
export const messageStartChangePage = (newPage) => {
    return (dispatch, getState) => {
        ...
        const state = getState();
        let currentPage = state.paginationReducer.bill.rejectedOut.page;
